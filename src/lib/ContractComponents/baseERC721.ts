...
export default class BaseERC721 {
  ...
  async transfer(params: TransferOptions): Promise<ethers.providers.TransactionResponse> {
    this.assertContractLoaded(Logger.location.BASEERC721_TRANSFER);
  ...
  async setApprovalForAll(
    ...
    this.assertContractLoaded(Logger.location.BASEERC721_SETAPPROVALFORALL);
  ...
  async approveTransfer(
    ...
    this.assertContractLoaded(Logger.location.BASEERC721_APPROVETRANSFER);
  ...
  async tokenURI(params: TokenURIOptions): Promise<string> {
    this.assertContractLoaded(Logger.location.BASEERC721_TOKEN_URI);

    if (!Number.isInteger(params.tokenId) || params.tokenId < 0) {
      log.throwArgumentError(Logger.message.tokenId_must_be_integer, 'tokenId', params.tokenId, {
        location: Logger.location.BASEERC721_TOKEN_URI,
      });
    }

    try {
      return this.contractDeployed.tokenURI(params.tokenId);
    } catch (error) {
      return log.throwError(Logger.message.ethers_error, Logger.code.NETWORK, {
        location: Logger.location.BASEERC721_TOKEN_URI,
        error,
  ...
  //   try {
  //     const options = addGasPriceToOptions({}, gas, Logger.location.BASEERC721_ADDGASPRICETOOPTIONS);
